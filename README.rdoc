...
* 清單段落
  ...
  可以在文章中<tt>新增<ul>清單</tt>。支援清單<tt>子項目<li>的修改</tt>及<tt>刪除</tt>。
...
* Peditor設定及預設值
    ...
    文字段落加入分類清單 - paragraphFontClass: { "內文": "content", "標題": "title" },（不生成清單：設定為false）
    文字段落加入字體色彩清單 - paragraphFontColor: { "顏色": "default", "黑色": "#000", "藍色": "#00F" },（不生成清單：設定為false）
    文字段落加入字體大小清單 - paragraphFontSize: { "大小": "default", 14:14, 28:28 }（不生成清單：設定為false）
